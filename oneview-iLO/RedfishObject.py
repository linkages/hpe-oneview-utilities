...
class RedfishObject(object):
    def __init__(self, host, token, loglevel):
        try:
            ...
            raise
        self.LOGGERFILE = "RedfishObject.log"
        self.LOGGERFORMAT = "%(asctime)s - %(name)s - %(levelname)s - %(message)s"
        self.LOGGER = redfish_logger(self.LOGGERFILE, self.LOGGERFORMAT, loglevel)
        self.typepath = redfish.ris.tpdefs.Typesandpathdefines()
        self.typepath.getgen(url=host, logger=self.LOGGER)
